...
//            System.out.println(Debugger.getCallerPosition()+" sample:"+e.toString());
            call_sample_data.updCountInfluencing_upzwt(e.u, e.z, e.w, e.time, -1); //update count

            int newZNormal = Probability.getMAPInfluencing_z(call_sample_data.cmdOption, call_sample_data, e.u, e.w, e.time);//getMAPCiting_zbop
            e.z = newZNormal;
            call_sample_data.updCountInfluencing_upzwt(e.u, e.z, e.w, e.time, +1); //update count

            double sampleProb = 0;
            for (int tmpZ=0; tmpZ< Constant.zNum; tmpZ++)
                sampleProb += Probability.influencingPosterior_z(e.u, e.w, tmpZ, e.time, call_sample_data, call_sample_data.cmdOption);
