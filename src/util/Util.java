...
public class Util{
	...
	public static int initiaInfluencing(List<Integer> bibliography) {
    ...
    public static void update3MapIncreamental(Map<Integer, Map<Integer, Map<Integer, Double>>> map,
        ...
        if(map1==null){
            if (map instanceof ConcurrentMap) {
                if (map instanceof ConcurrentHashMap)
                    map1 = new ConcurrentHashMap<Integer, Map<Integer, Double>>();
                else if (map instanceof ConcurrentSkipListMap)
                    map1 = new ConcurrentSkipListMap<Integer, Map<Integer, Double>>();
    ...
    public static void update2MapIncreamental(Map<Integer, Map<Integer, Double>> map,
        ...
        if(map1==null){
            if (map instanceof ConcurrentMap) {
                if (map instanceof ConcurrentHashMap)
                    map1 = new ConcurrentHashMap<Integer, Double>();//
                else if (map instanceof ConcurrentSkipListMap)
                    map1 = new ConcurrentSkipListMap<Integer, Double>();//
    ...
    public static void update1MapIncreamental(Map<Integer, Double> map, int key, double value){
        double oldFreq = map.containsKey(key)? map.get(key) : 0;
...
//            N_uztb=0
            bw = new BufferedWriter(new FileWriter(new File(outputfile_prefix+".tau_innov")));
            bw.append("U \t Z \t T(b=0) \t freq\n");
            Util.print3Map(sampleData.N_uzt_innov_influenced, "", bw);
            bw.flush();
            bw.close();

            //            N_uup influenced
            bw = new BufferedWriter(new FileWriter(new File(outputfile_prefix+".gamma")));
            ...
            //            N_uupz influenced
            bw = new BufferedWriter(new FileWriter(new File(outputfile_prefix+".gamma_z")));
            bw.append("U \t Up \t Z influenced \t freq\n");
            Util.print3Map(sampleData.N_uupz_inher_influenced, "", bw);
            bw.flush();
            bw.close();

            //            N_uupzt influenced
            bw = new BufferedWriter(new FileWriter(new File(outputfile_prefix+".tau_inher")));
            bw.append("U \t Up \t Z \t T influenced \t freq\n");
            Util.print4Map(sampleData.N_uupzt_inher_influenced, bw);
            bw.flush();
            bw.close();;
...
//            N_upzt influencing
            bw = new BufferedWriter(new FileWriter(new File(outputfile_prefix+".tau_prime")));
            bw.append("Up \t Z \t T influencing  \t freq\n");
            Util.print3Map(sampleData.N_upzt_influencing, "", bw);
            bw.flush();
            bw.close();

//            N_zw_all.  Print z -> word index-> word string
            bw = new BufferedWriter(new FileWriter(new File(outputfile_prefix+".phi")));
            bw.append("Z \t W_index \t freq\n");
            Util.print2Map(sampleData.N_zw_all, "", bw);
            bw.flush();
            bw.close();
